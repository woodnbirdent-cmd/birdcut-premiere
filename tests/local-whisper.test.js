...
describe("Local Whisper sidecar protocol", () => {
  it("strips /v1 for GET /health", () => {
    ...
    assert.equal(healthRoot("http://127.0.0.1:8090/v1/"), "http://127.0.0.1:8090");
    assert.match(sidecarNotRunningMessage("http://127.0.0.1:8090/v1"), /Local Whisper sidecar is not running/i);
  });
  ...
  it("maps a connection failure to the start-sidecar message", async () => {
    await assert.rejects(
      ...
      /Local Whisper sidecar is not running/i
    );
  });
...
describe("Local Whisper panel", () => {
  ...
  it("surfaces Start Local Whisper sidecar when health fails", async () => {
    const origFetch = global.fetch;
    global.fetch = async () => {
      throw new Error("Failed to fetch");
    };
    try {
      ...
      await controller.mount();
      assert.match(root.innerHTML, /Start Local Whisper sidecar/);
      await controller.transcribe("sequence");
