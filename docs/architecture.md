...
| `local-whisper` | Bounce sequence/clip with audio `.epr`, then `POST http://127.0.0.1:8090/v1/audio/transcriptions` (faster-whisper sidecar). **No API key.** `GET /health` — panel shows **Start Local Whisper sidecar** if the process is down. Word times map like Whisper HTTP. |
| `adobe` | Premiere native Speech to Text: `Transcript.transcribeClipProjectItem` on each source `ClipProjectItem`, then `exportToJSON`. Prefers Project-panel `ClipProjectItem` selection; skips nested sequences. Language is passed only when `querySupportedLanguages` lists it **and** `isLanguagePackAvailable` is not false (otherwise omit options — Premiere default). `hasTranscript` true → export only. **Import Premiere transcript** is export-only. Error `-1609629681` / `0xa00f000f` maps to a packs / Project-panel / Text-panel-import message. **Optional — broken on some Premiere 26.5 Mac installs** even with SpeechESL; use Local Whisper. JSON follows Adobe’s published spec. **No OpenAI key.** Cloud languages may still use Adobe credits. |
