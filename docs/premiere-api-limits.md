...
| Native STT | `Transcript.transcribeClipProjectItem`, `exportToJSON`, `hasTranscript`, `querySupportedLanguages`, `isLanguagePackAvailable` | Adobe Speech to Text is **per source clip**, not per sequence mix. Prefer `ProjectUtils.getSelection` → `ClipProjectItem.cast` (Adobe sample `transcript.ts` fires transcribe as a direct Promise, not `executeTransaction`). Passing an unsupported `language` throws generic **-1609629681** (`0xa00f000f`). Nested sequences cannot be transcribed. **Optional on Premiere 26.5 Mac** — some installs fail even with SpeechESL; BirdCut’s Local Whisper sidecar is the unblock path. JSON spec: [transcript_format_spec.json](https://github.com/AdobeDocs/uxp-premiere-pro-samples/blob/main/sample-panels/premiere-api/assets/transcript_format_spec.json). |
