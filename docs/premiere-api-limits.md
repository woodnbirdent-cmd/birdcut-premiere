...
6. **Caption track create/edit is incomplete.** BirdCut’s **Add captions to sequence** imports SRT/TTML and inserts the ProjectItem (`importFiles` + `createInsertProjectItemAction`). Adobe has not shipped `createCaptionTrack()`, caption-segment, or MOGRT source-text APIs. 1–2 word cues use transcript word times; panel preview can scale, the timeline cannot. Style is SRT/TTML font/color/outline. Export SRT remains available.
