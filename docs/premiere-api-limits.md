...
| Captions on sequence | `project.importFiles` + `SequenceEditor.createInsertProjectItemAction` | Writes `.srt` / TTML `.xml` to the UXP temp folder, imports, then inserts the ProjectItem. There is still **no** `createCaptionTrack()`. If insert does not land on a caption track, drag the imported file from the Project panel. |
...
6. **Caption track create/edit is incomplete.** BirdCut’s **Add captions to sequence** imports SRT/TTML and inserts the ProjectItem (`importFiles` + `createInsertProjectItemAction`). Adobe has not shipped `createCaptionTrack()` or caption-segment APIs. Style presets are encoded in SRT align/font tags and TTML; Premiere may flatten some of that. Karaoke/pop are **word/phrase timing**, not After Effects motion. Export SRT remains available.
