...
| Native STT | `Transcript.transcribeClipProjectItem`, `exportToJSON`, `hasTranscript`, `querySupportedLanguages` | Adobe Speech to Text is **per source clip**, not per sequence mix. JSON spec: [transcript_format_spec.json](https://github.com/AdobeDocs/uxp-premiere-pro-samples/blob/main/sample-panels/premiere-api/assets/transcript_format_spec.json). Nested sequences cannot be transcribed directly. Optional on Premiere 26.5 Mac if native STT fails. |
| Local Whisper | UXP `fetch` → `http://127.0.0.1:8090/v1/audio/transcriptions` | Panel cannot run ML. `sidecar/local-whisper` (faster-whisper) implements OpenAI `verbose_json` + word timestamps. Bounce still needs an audio `.epr`. |
...
| Captions on sequence | `project.importFiles` + `SequenceEditor.createInsertProjectItemAction` | Writes styled **`.ttml`** (preferred) and `.srt` to the UXP temp folder, imports TTML first, then inserts the ProjectItem. There is still **no** `createCaptionTrack()`. If insert does not land on a caption track, drag the `.ttml` from the Project panel. Premiere’s SRT importer usually ignores font/color. |
| Captions track | `sequence.getCaptionTrack` / `getCaptionTrackCount` | Can list/rename tracks (26.3+ rename). Cannot create segments or set caption style via UXP. |
...
6. **Caption track create/edit is incomplete.** BirdCut’s **Add captions to sequence** imports TTML (`.ttml`) then SRT (`importFiles` + `createInsertProjectItemAction`). Adobe has not shipped `createCaptionTrack()`, caption-segment, or MOGRT source-text APIs. 1–2 word cues use transcript word times; panel preview can scale, the timeline cannot. Style is TTML font/color/outline plus SRT `<font face>` tags. Premiere often applies its **default caption look** on SRT import and may strip TTML styles. **ALL CAPS** is uppercase letters in the file. Color UI is swatches + hex because UXP `input type="color"` is broken in Premiere. Export SRT remains available.
