    ...
    try {
      ...
      if (provider === "local-whisper") {
        ...
        if (state.localWhisperHealth && !state.localWhisperHealth.ok) {
          throw new Error(state.localWhisperHealth.message || "Start the Local Whisper sidecar");
        }
  ...
  function renderSettings() {
    ...
    return h(`
      <form id="settings-form" class="settings">
        ...
        ${
          s.sttProvider === "adobe"
            ? `<p class="muted">Uses Premiere’s Speech to Text on each source clip. No OpenAI key. On-device packs (when installed) stay local; Adobe cloud languages may still use Adobe credits. On some Premiere 26.5 Mac installs this path fails — use Local Whisper instead. ${
            ...
            ? `<p class="muted">Bounces the sequence/clip with your audio-only .epr, then sends it to a Whisper sidecar on this Mac. No API key. Default URL <code>http://127.0.0.1:8090/v1</code>.</p>
               <p class="${state.localWhisperHealth && state.localWhisperHealth.ok ? "muted" : "muted"}">${
                 state.localWhisperHealth && state.localWhisperHealth.ok
                   ...
                   : "Start the Local Whisper sidecar — Terminal: <code>cd sidecar/local-whisper && ./start.sh</code>"
               }</p>
